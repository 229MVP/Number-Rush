...
import { resetTemplateStats } from '../storage/templateStatsStorage';
...
export function SettingsScreen({ navigation }: Props) {
  ...
  async function handleResetBestScore() {
    ...
    Alert.alert(
      'Reset Best Score',
      'This clears your locally saved best score. This cannot be undone.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            Alert.alert('Reset', 'Best score has been reset to 0.');
          },
        },
      ],
    );
  ...
  function handleResetAllPress() {
    ...
    Alert.alert(
      ...
      'This will permanently delete your best score, statistics, tutorial progress, and settings on this device. This action cannot be undone.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Proceed',
    ...
    <View style={[styles.root, { paddingTop: insets.top }]} testID="screen-settings">
      ...
      >
        ...
        <View style={styles.section}>
          <ToggleRow
            ...
            onToggle={() => {
              tap();
              void patchSettings({ confirmPowerUpUse: !settings.confirmPowerUpUse });
            }}
            accent={colors.orange}
          />
          <NeonButton
        ...
        <View style={styles.section}>
          ...
          />
          <NeonButton
            testID="settings-reset-best-score"
            label="RESET BEST SCORE"
            color={colors.orange}
            size="small"
            onPress={() => { void handleResetBestScore(); }}
          />
          <NeonButton
        ...
        <View style={styles.section}>
          ...
          >
            <Text style={styles.rowLabel}>About Number Rush</Text>
      ...
      >
        <View style={styles.overlay}>
          <View style={[styles.modalBox, neonGlow(ACCENT, 10)]}>
            <Text style={[styles.modalTitle, { color: ACCENT }]}>NUMBER RUSH</Text>
            <Text style={styles.modalBody}>
              A neon arcade number-puzzle template built with Expo, React
              Native, and TypeScript. Progress and settings are saved locally
              on this device only.
            </Text>
            <Text style={styles.modalBody}>Version {getAppVersion()}</Text>
            <NeonButton
              label="CLOSE"
              color={ACCENT}
              size="small"
