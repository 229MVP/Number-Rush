...
export function useNumberRushGame({
  ...
  const loadInventoryAndRestart = useCallback(
    async (config: RunConfiguration) => {
      ...
      if (config.powerUpsEnabled && TEMPLATE_FEATURES.shop) {
        // Shop/economy build: persisted inventory carries across runs.
        ...
        // Sellable template: fixed per-run defaults (Multiplier ×2, Swap ×3,
        // Bomb/Freeze/Shield/Wild ×0) — no shop or saved economy required.
