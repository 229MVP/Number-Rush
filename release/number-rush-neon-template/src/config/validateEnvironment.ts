...
/** True when every backend-connected template flag is disabled (sellable-template posture). */
function isTemplateOfflineMode(): boolean {
  return (
    !TEMPLATE_FEATURES.accounts &&
    !TEMPLATE_FEATURES.cloudSync &&
    !TEMPLATE_FEATURES.ranked &&
    !TEMPLATE_FEATURES.dailyTournament &&
...
export function validateEnvironment(): EnvironmentValidation {
  ...
  // Sellable-template posture: no backend features requested, so Supabase
  // and connected-feature warnings would only confuse a template buyer.
  if (isTemplateOfflineMode()) {
    if (env === 'development') {
      logger.info('Running as sellable template (Classic-only, no backend).');
    }
